...
output$global_network = renderVisNetwork({
  ...
  data$nodes$color.background = case_when(
    data$nodes$Group == "Male" ~ '#0048E4',
    data$nodes$Group == "Female" ~ '#FF0000',
    data$nodes$Group == "Place" ~ '#62BC0A',
    data$nodes$Group == "God" ~ '#FBF100'
  )
  ...
  data$edges$color.color = case_when(
    data$edges$Relation == "is teacher of" ~ '#55402B',
    data$edges$Relation == "is friend of" ~ '#4F66AD',
    data$edges$Relation == "is family of" ~ '#FF0000', 
    data$edges$Relation == "studied the work of" ~ '#FC8A21',
    data$edges$Relation == "sent letters to" ~ '7B09B0',
    #data$edges$Relation == "is associated with" ~ '#14ff03',
    data$edges$Relation == "is benefactor of" ~ '#9C8475'
  )
  ...
  lnodes <- data.frame(label = c("Male", "Female"),
                       shape = c( "dot"), 
                       color = c("#0048E4", "#FF0000"),
                       id = 1:2)
  
  if (directed) arrow = c("to", FALSE, FALSE, "to", "to") else  arrow = rep(as.character(FALSE),5) 
  
  # edges data.frame for legend
  ledges <- data.frame(color = c("#55402B", "#4F66AD", "#FF0000", '#FC8A21','7B09B0'),
...
output$local_network = renderVisNetwork({
  ...
  if (!is.null(input$node_sel) && !is.null(input$order)) {
    ...
    data$edges$color.color = case_when(
      data$edges$Relation == "is teacher of" ~ '#55402B',
      data$edges$Relation == "is friend of" ~ '#4F66AD',
      data$edges$Relation == "is family of" ~ '#3A6F06', 
      data$edges$Relation == "studied the work of" ~ '#FC8A21',
      data$edges$Relation == "sent letters to" ~ '7B09B0',
      #data$edges$Relation == "is associated with" ~ '#14ff03',
      data$edges$Relation == "is benefactor of" ~ '#9C8475'
