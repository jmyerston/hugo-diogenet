...
output$global_network = renderVisNetwork({
  ...
  data$nodes$color.background = case_when(
    data$nodes$Group == "Male" ~ '#008080',
    data$nodes$Group == "Female" ~ '#BC3B0A',
    data$nodes$Group == "Place" ~ '#836530',
    data$nodes$Group == "God" ~ '#FCBC21'
  )
  ...
  data$edges$color.color = case_when(
    data$edges$Relation == "is teacher of" ~ '#FC5821',
    data$edges$Relation == "is friend of" ~ '#405DDD',
    data$edges$Relation == "is family of" ~ '#55BF2B', 
    data$edges$Relation == "studied the work of" ~ '#AE6400',
    data$edges$Relation == "sent letters to" ~ '#DF5670',
    #data$edges$Relation == "is associated with" ~ '#14ff03',
    data$edges$Relation == "is benefactor of" ~ '#7F8D9E'
  )
  ...
  lnodes <- data.frame(label = c("Male", "Female"),
                       shape = c( "dot"), 
                       color = c("#008080", "#BC3B0A"),
                       id = 1:2)
  
  if (directed) arrow = c("to", FALSE, FALSE, "to", "to") else  arrow = rep(as.character(FALSE),5) 
  
  # edges data.frame for legend
  ledges <- data.frame(color = c("#FC5821", "#405DDD", "#55BF2B", '#AE6400','#ffbf00'),
